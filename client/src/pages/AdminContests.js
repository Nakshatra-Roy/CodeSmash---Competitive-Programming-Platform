...
import AdminContestFilters from "../components/AdminContestFilters";
...
const AdminContests = () => {
  ...
  const filteredContests = useMemo(() => {
    ...
    return contests.filter(c => {
      if (filters.showPending && c.status !== "Pending") return false;
      if (filters.showRejected && c.status !== "Rejected") return false;
  ...
  const handleReset = () => setFilters({ search: "", tag: "", organizer: "", startDate: "", showPending: false });

  const handleShowPending = () => setFilters(prev => ({ ...prev, showPending: !prev.showPending }));
        ...
        <AdminContestFilters
          ...
          extraButtons={
            <>
              ...
              <button
                className={`btn small ghost${filters.showPending ? " active" : ""}`}
